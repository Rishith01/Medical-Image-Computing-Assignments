...
In this experiment, the Bayesian image denoising framework was extended to a vector-valued RGB microscopy image provided in the \texttt{data} folder. The noise was assumed to follow a Gaussian distribution as an approximation. Unlike Part~2, where grayscale images were considered, this task required joint denoising of the three RGB channels using vector-valued Markov Random Field (MRF) priors.

Three different vector-valued MRF prior functions were investigated:

\begin{itemize}
\item \textbf{Prior A (Squared-$\ell_2$ prior):} the penalty between two neighboring pixels is the squared $\ell_2$ norm of their vector difference.
\item \textbf{Prior B ($\ell_2$ prior):} the penalty between two neighboring pixels is the $\ell_2$ norm of their vector difference.
\item \textbf{Prior C (Huber-regularized $\ell_1$ prior):} the penalty between two neighboring pixels is the Huber-regularized $\ell_1$ norm of their vector difference.
\end{itemize}

Regularization was applied jointly across all RGB channels. For each prior, the optimal regularization parameters were determined by minimizing the relative root mean square error (RRMSE) with respect to the noiseless reference image.

\subsection*{(a) Visual Denoising Results and Edge Preservation}

Figure~\ref{fig:rgb_results} shows the five required images, displayed using the \textbf{same jet colormap} and visualizing the magnitude of the RGB pixel values at each location:
(i) noiseless image,
(ii) noisy image,
(iii) denoised image using Prior~A,
(iv) denoised image using Prior~B,
(v) denoised image using Prior~C.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{all_image_q3.png}
\caption{(i) Noiseless RGB microscopy image, (ii) noisy image, (iii) denoised image using squared-$\ell_2$ prior (A), (iv) denoised image using $\ell_2$ prior (B), and (v) denoised image using Huber-$\ell_1$ prior (C). All images are shown using the same jet colormap.}
\label{fig:rgb_results}
\end{figure}

\textbf{Edge Preservation Observation:}  
Edge preservation is most clearly observed in cases \textbf{B} and \textbf{C}, with the strongest preservation occurring for \textbf{Prior~C (Huber-$\ell_1$)}.

\textbf{Explanation:}  
The squared-$\ell_2$ prior (A) penalizes large gradients heavily, leading to excessive smoothing across edges.  
The $\ell_2$ prior (B) reduces the penalty on large vector differences, allowing sharper transitions to be preserved.  
The Huber-regularized $\ell_1$ prior (C) behaves quadratically for small gradients and linearly for large gradients, which limits smoothing across high-contrast boundaries while still suppressing noise in homogeneous regions. This results in superior edge preservation.

\subsection*{(b) Objective Function Convergence}

Figure~\ref{fig:convergence} shows the objective function value versus iteration for the three denoising cases using their respective optimal parameters.


\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{convergence.png}
\caption{Objective function value versus iteration for squared-$\ell_2$ prior (A) , $\ell_2$ prior (B) and  Huber-$\ell_1$ prior (C).}
\label{fig:convergence}
\end{figure}

In all three cases, the objective function increases monotonically and converges to a stable value, indicating successful optimization.
